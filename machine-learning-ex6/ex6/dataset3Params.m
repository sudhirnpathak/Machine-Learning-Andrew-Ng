...
predictionError = zeros(length(Cvalues), length(sigmaValues));

for i = 1:length(Cvalues)
for j = 1:length(sigmaValues)
	currentC = Cvalues(i);
	currentSigma = sigmaValues(j);
...
minError = min(min(predictionError));
[rindex, cindex] = find(predictionError == minError);

C = Cvalues(rindex);
sigma = sigmaValues(cindex); 
